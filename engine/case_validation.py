"""Whole-case smoke-only validation with NIfTI and diagnostic outputs."""
...
def validate_cases(model: nn.Module, raw_root: Path, case_ids: tuple[str, ...], output_root: Path, device: torch.device, *, fold: int, checkpoint_path: Path) -> list[dict[str, object]]:
    """Validate explicit cases; all generated files stay below the supplied output root."""
    validation = output_root / "validation"; predictions = validation / "predictions"; records: list[dict[str, object]] = []
    for case_id in case_ids:
        image = read_nifti(raw_root / "imagesTr" / f"{case_id}_0000.nii.gz")
        label = read_nifti(raw_root / "labelsTr" / f"{case_id}.nii.gz")
        prediction = predict_volume(model, image, device)
        prediction_path = predictions / f"{case_id}.nii.gz"
        save_and_validate_prediction(prediction_path, prediction, label)
        record = {"case_id": case_id, **volume_metrics(prediction, label.array.astype("uint8"))}
