...
def capture_standalone_inference(
    *,
    ...
    if input_mode is InputMode.DWI_ADC_FUSION:
        prepared = prepare_dwi_adc_fusion_case(raw_root, case_id)
        prediction = restore_dwi_adc_fusion_prediction(
            prepared,
            np.asarray(
                predict_volume(
                    model,
    ...
    elif input_mode is InputMode.DWI_ADC_BILATERAL:
