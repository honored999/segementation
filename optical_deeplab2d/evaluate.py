...
def main() -> None:
 ...
 for r in read_manifest(a.data_root):
  if r.patient in fold.val_patients: x,y=load_sample(r);rows.append({'patient':r.patient,'target':y.numpy(),'prediction':(m(x).sigmoid().detach().numpy()>=c['threshold'])})
 print(write_evaluation(rows,a.output_dir))
